...
class Board_Ext():
    ...
    def last_step(self,board):
        ...
        for p in self._grid_ext.keys():
            if self._grid_ext[p][0] > num_last_step:
                num_last_step = self._grid_ext[p][0]

        return num_last_step

    def place_stone_ext(self, board, player, point=None):
        if point==None:
            return False
        self.steps = self.last_step(board)+1
        self._grid_ext[point] = [self.steps, player, 1]
