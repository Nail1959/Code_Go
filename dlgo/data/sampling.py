...
class Sampler:
    ...
    def load_sample_set(self):  # Nail Read existed train and test .npy files"
        sample_set = glob.glob(self.data_dir+'/KGS*.gz')
    ...
    def draw_samples(self, num_sample_games):
        ...
        #sample_set = self.load_sample_set(self)  # Nail  old value set()
        sample_set=set()
    ...
    def draw_training_samples(self, num_sample_games):
        ...
        #sample_set_exist = self.load_sample_set()  # Nail  old value set()
        sample_set = set()
        while len(sample_set) < num_sample_games:
            sample = random.choice(available_games)
            if sample not in self.test_games:
