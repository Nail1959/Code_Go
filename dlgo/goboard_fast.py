...
class Board_Ext():
    ...
    def __init__(self, board):
        ...
        self.steps_b = 0
        self.steps_w = 0
        self.board_size = self.num_rows * self.num_cols
        #self.dlta = 1.0 / self.board_size
        self.dlta = 2.0 / (self.board_size)   # Every Player can has max stones on board
        for p in board._grid.keys():
            self._grid_ext[p] = [0, board._grid[p], 0]
    ...
    def check_board(self, board):
        ...
        for point in grid.keys():
            if grid[point] is None:
                try:
                    s = self._grid_ext.pop(point, 0)
                except:
                    pass

    def find_liberties_point(self):
        #go_string = game_state.board.get_go_string(p)
        return True

    def last_step(self, board):
        num_last_step_b = 0
        num_last_step_w = 0
        self.check_board(board)  # Delete elements
        for p in self._grid_ext.keys():
            if self._grid_ext[p][1] == 'b':
                if self._grid_ext[p][0] > num_last_step_b:
                    num_last_step_b = self._grid_ext[p][0]
            if self._grid_ext[p][1] == 'w':
                if self._grid_ext[p][0] > num_last_step_w:
                    num_last_step_w = self._grid_ext[p][0]

        return num_last_step_b, num_last_step_w

    def place_stone_ext(self, board, player, point=None):
        if point == None:
            return False
        self.steps_b, self.steps_w = self.last_step(board)
        if player == 'b':
            self.steps_b += 1
            self._grid_ext[point] = [self.steps_b, player, 1]
        if player == 'w':
            self.steps_w += 1
            self._grid_ext[point] = [self.steps_w, player, -1]

        for p in self._grid_ext.keys():
            if p == point:
                continue
            if self._grid_ext[p][1] == 'b':
                self._grid_ext[p][2] = self._grid_ext[p][2] - self.dlta
                if self._grid_ext[p][2] <= 0:
                    self._grid_ext[p][2] = self.dlta
            if self._grid_ext[p][1] == 'w':
                self._grid_ext[p][2] = self._grid_ext[p][2] + self.dlta
                if self._grid_ext[p][2] >= 0:
                    self._grid_ext[p][2] = -self.dlta
    ...
    def get(self, point):
        try:
            player = self._grid_ext[point][1].name
            cost = self._grid_ext[point][2]
            return player, cost
        except:
            return None, 0.000

    def find_value(self, value=1):
        for row in range(1, self.num_rows + 1):
            for col in range(1, self.num_cols + 1):
                point = Point(row, col)
                if self._grid_ext[point][2] == value:
                    return point
        return None
    ...
    def find_max_value(self):
        ...
        for row in range(1, self.num_rows + 1):
            for col in range(1, self.num_cols + 1):
                point = Point(row, col)
                try:
                    if self._grid_ext[point][2] > max_value:
                        max_value = self._grid_ext[point][2]
                        p_max = point
                except:
                    continue
        return max_value, p_max
    ...
    def find_min_value(self):
        ...
        for row in range(1, self.num_rows + 1):
            for col in range(1, self.num_cols + 1):
                point = Point(row, col)
                try:
                    if self._grid_ext[point][2] < min_value:
                        min_value = self._grid_ext[point][2]
                        p_min = point
                except:
                    continue
        return min_value, p_min

    def count_stones(self):  # Count stones on board
        try:
            return len(self._grid_ext)
        except:
            return 0
